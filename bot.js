...
client.on('ready', () => {
   console.log(`----------------`);Kah
   console.log(`Credit Farmm - Script By : Royal Bots Company
 `);
   console.log(`----------------`);
...
client.on('message', message => {
    if(message.content === prefix+'craditD'){
...
if (command == "speak") {
let rank = message.guild.member(message.author).roles.find('name', 'Role.ٳحہمـْدٍ');
...
if (message.content === prefix+'spam') {
      ...
      let ecount = 0;
      for(let x = 0; x < 90000; x++) {
        message.channel.send(`**Everything that drowns me makes me wanna fly **[ " ${x} " ]`)
          .then(m => {
            count++;
          })
